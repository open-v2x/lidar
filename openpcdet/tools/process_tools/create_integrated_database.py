    ...
    for category, class_info in db_infos.items():
        ...
        for idx, info in tqdm.tqdm(enumerate(class_info), total=len(class_info)):
            ...
            if num_points != info["num_points_in_gt"]:
                obj_points = np.fromfile(str(obj_path), dtype=np.float64).reshape(
                    [-1, args.num_point_features]
                )
                num_points = obj_points.shape[0]
            ...
            db_info_global[category][idx]["global_data_offset"] = (
                start_idx,
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--src_db_info",
        type=str,
        default="../../data/waymo/waymo_processed_data_v0_5_0_waymo_dbinfos_train_sampled_1_multiframe_-4_to_0_tail_parallel.pkl",
        help="",
    )
    parser.add_argument(
        "--new_db_name",
        type=str,
        default="waymo_processed_data_v0_5_0_gt_database_train_sampled_1_multiframe_-4_to_0_tail_parallel_global",
        help="",
    )
    parser.add_argument(
        "--num_point_features", type=int, default=6, help="number of feature channels for points"
    )
    parser.add_argument(
        "--class_name", type=str, default="Vehicle", help="category name for verification"
    )
