...
class CenterHead(nn.Module):
    def __init__(
        ...
        self.conv_cls = nn.Conv2d(input_channels, self.num_class, kernel_size=1)
        self.conv_box = nn.Conv2d(input_channels, 8, kernel_size=1)
            ...
            for k in range(num_objs):
                ...
                if width > 0 and length > 0:
                    radius = gaussian_radius(
                        (length, width), min_overlap=self.target_cfg.GAUSSIAN_OVERLAP
                    )
                    ...
                    coor_x = (x - pc_range[0]) / voxel_size[0] / self.target_cfg.OUT_SIZE_FACTOR
    ...
    def generate_predicted_boxes(self, batch_size, cls_preds, box_preds, dir_cls_preds=None):
        ...
        xs = (
            xs * self.target_cfg.OUT_SIZE_FACTOR * self.target_cfg.VOXEL_SIZE[0]
            + self.point_cloud_range[0]
        )
        ys = (
            ys * self.target_cfg.OUT_SIZE_FACTOR * self.target_cfg.VOXEL_SIZE[1]
    ...
    def get_cls_layer_loss(self):
        ...
        cls_loss = self.loss_cls(pred_heatmaps, gt_heatmaps, avg_factor=max(num_pos, 1))
    ...
    def get_box_reg_layer_loss(self):
        ...
        target_box, inds, masks = (
            self.forward_ret_dict["anno_boxes"][0],
            self.forward_ret_dict["inds"][0],
            self.forward_ret_dict["masks"][0],
        )
        ...
        loc_loss = l1_loss(pred, target_box, bbox_weights, avg_factor=(num + 1e-4))
