...
class AxisAlignedTargetAssigner(object):
    def __init__(self, model_cfg, class_names, box_coder, match_height=False):
        ...
        self.pos_fraction = (
            anchor_target_cfg.POS_FRACTION if anchor_target_cfg.POS_FRACTION >= 0 else None
        )
    ...
    def assign_targets(self, all_anchors, gt_boxes_with_classes):
        ...
        for k in range(batch_size):
            ...
            if self.use_multihead:
                ...
                target_dict = {
                    "box_cls_labels": [
                        t["box_cls_labels"].view(*feature_map_size, -1) for t in target_list
                    ],
                    "box_reg_targets": [
                        t["box_reg_targets"].view(*feature_map_size, -1, self.box_coder.code_size)
                        for t in target_list
                    ],
                    "reg_weights": [
                        t["reg_weights"].view(*feature_map_size, -1) for t in target_list
                    ],
                }
    ...
    def assign_targets_single(
        self, anchors, gt_boxes, gt_classes, matched_threshold=0.6, unmatched_threshold=0.45
    ):
