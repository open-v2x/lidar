...
class CenterHead(nn.Module):
    def __init__(
        ...
        for cur_class_names in self.model_cfg.CLASS_NAMES_EACH_HEAD:
            ...
            cur_class_id_mapping = torch.from_numpy(
                np.array([self.class_names.index(x) for x in cur_class_names if x in class_names])
            ).cuda()
    ...
    def get_loss(self):
        ...
        for idx, pred_dict in enumerate(pred_dicts):
            ...
            loc_loss = (
                reg_loss
                * reg_loss.new_tensor(self.model_cfg.LOSS_CONFIG.LOSS_WEIGHTS["code_weights"])
            ).sum()
