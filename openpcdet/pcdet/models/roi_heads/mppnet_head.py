...
class ProposalTargetLayerMPPNet(ProposalTargetLayer):
    ...
    def forward(self, batch_dict):
        ...
        if self.roi_sampler_cfg.CLS_SCORE_TYPE == "cls":
            ...
            batch_cls_labels[interval_mask] = (batch_roi_ious[interval_mask] - iou_bg_thresh) / (
    ...
    def sample_rois_for_mppnet(self, batch_dict):
        ...
        batch_gt_of_rois = rois.new_zeros(
            batch_size, self.roi_sampler_cfg.ROI_PER_IMAGE, gt_boxes.shape[-1]
        )
        ...
        batch_trajectory_rois = rois.new_zeros(
            batch_size,
        ...
        batch_valid_length = rois.new_zeros(
            (
                batch_size,
                batch_dict["trajectory_rois"].shape[1],
                self.roi_sampler_cfg.ROI_PER_IMAGE,
            )
        )
            ...
            if self.roi_sampler_cfg.USE_TRAJ_AUG.ENABLED:
                ...
                for idx in range(0, batch_dict["num_frames"]):
                    if idx == cur_frame_idx:
                        ...
                        continue
                    fg_trajs, _ = self.aug_roi_by_noise_torch(
                    ...
                    batch_trajectory_rois_list.append(
                        torch.cat([fg_trajs, bg_trajs], 0)[None, :, :]
                    )
    ...
    def aug_roi_by_noise_torch(
        self, roi_boxes3d, gt_boxes3d, iou3d_src, aug_times=10, pos_thresh=None
    ):
    ...
    def random_aug_box3d(self, box3d):
        ...
        if self.roi_sampler_cfg.REG_AUG_METHOD == "single":
            ...
            hwl_scale = ((torch.rand(3, device=box3d.device) - 0.5) / 0.5) * range_config[idx][
                1
            ] + 1.0
            angle_rot = ((torch.rand(1, device=box3d.device) - 0.5) / 0.5) * range_config[idx][2]

            aug_box3d = torch.cat(
                [box3d[0:3] + pos_shift, box3d[3:6] * hwl_scale, box3d[6:7] + angle_rot], dim=0
            )
            return aug_box3d
...
class MPPNetHead(RoIHeadTemplate):
    def __init__(self, model_cfg, num_class=1, **kwargs):
        ...
        self.proposal_target_layer = ProposalTargetLayerMPPNet(
        ...
        self.jointembed = MLP(
            self.hidden_dim * (self.num_groups + 1),
        ...
        self.up_dimension_geometry = MLP(
            input_dim=29, hidden_dim=64, output_dim=hidden_dim // num_radius, num_layers=3
        )
        self.up_dimension_motion = MLP(
            input_dim=30, hidden_dim=64, output_dim=hidden_dim, num_layers=3
        )
        ...
        if self.model_cfg.Transformer.use_grid_pos.enabled:
            if self.model_cfg.Transformer.use_grid_pos.init_type == "index":
                ...
                self.grid_pos_embeded = MLP(
                    input_dim=3, hidden_dim=256, output_dim=hidden_dim, num_layers=2
                )
            else:
    ...
    def roi_grid_pool(
        self, batch_size, rois, point_coords, point_features, batch_dict=None, batch_cnt=None
    ):
        ...
        point_coords = point_coords.view(
            point_coords.shape[0] * num_frames,
            point_coords.shape[1] // num_frames,
        ...
        features = pooled_features.view(
            point_features.shape[0], num_frames * self.num_proxy_points, pooled_features.shape[-1]
        ).contiguous()

        return (
            features,
            global_roi_proxy_points.view(
                batch_size * rois.shape[2], num_frames * num_proxy_points, 3
            ).contiguous(),
        )
    ...
    def crop_current_frame_points(self, src, batch_size, trajectory_rois, num_rois, batch_dict):
        ...
        for bs_idx in range(batch_size):
            ...
            cur_radiis = (
                torch.sqrt((cur_batch_boxes[:, 3] / 2) ** 2 + (cur_batch_boxes[:, 4] / 2) ** 2)
                * 1.1
            )
            ...
            dis = torch.norm(
                (
                    ...
                    - cur_batch_boxes[:, :2].unsqueeze(1).repeat(1, cur_points.shape[0], 1)
                ),
                dim=2,
            )
    ...
    def crop_previous_frame_points(
        self, src, batch_size, trajectory_rois, num_rois, valid_length, batch_dict
    ):
        for bs_idx in range(batch_size):
            ...
            for idx in range(1, trajectory_rois.shape[1]):
                ...
                if not self.training and cur_batch_boxes.shape[0] > 32:
                    ...
                    for i in range(length_iter + 1):
                        dis = torch.norm(
                            (
                                cur_time_points[:, :2].unsqueeze(0)
                                - cur_batch_boxes[32 * i : 32 * (i + 1), :2]
                                .unsqueeze(1)
                                .repeat(1, cur_time_points.shape[0], 1)
                            ),
                            dim=2,
                        )
                        dis_list.append(dis)
                ...
                for roi_box_idx in range(0, num_rois):
                    ...
                    if cur_roi_points.shape[0] > self.num_lidar_points:
                        ...
                        choice = np.random.choice(
                            cur_roi_points.shape[0], self.num_lidar_points, replace=True
                        )
                    ...
                    src[
                        bs_idx,
                        roi_box_idx,
                        self.num_lidar_points * idx : self.num_lidar_points * (idx + 1),
                        :,
                    ] = cur_roi_points_sample

        return src

    def get_proposal_aware_geometry_feature(
        self, src, batch_size, trajectory_rois, num_rois, batch_dict
    ):
        proposal_aware_feat_list = []
        for i in range(trajectory_rois.shape[1]):

            corner_points, _ = self.get_corner_points_of_roi(
            ...
            trajectory_roi_center = (
                trajectory_rois[:, i, :, :].contiguous().reshape(batch_size * num_rois, -1)[:, :3]
            )
            corner_add_center_points = torch.cat([corner_points, trajectory_roi_center], dim=-1)
            proposal_aware_feat = src[
            ...
            lwh = (
                trajectory_rois[:, i, :, :]
                .reshape(batch_size * num_rois, -1)[:, 3:6]
                .unsqueeze(1)
                .repeat(1, proposal_aware_feat.shape[1], 1)
            )
            diag_dist = (lwh[:, :, 0] ** 2 + lwh[:, :, 1] ** 2 + lwh[:, :, 2] ** 2) ** 0.5
            proposal_aware_feat = self.spherical_coordinate(
                proposal_aware_feat, diag_dist=diag_dist.unsqueeze(-1)
            )
        ...
        proxy_point_geometry, proxy_points = self.roi_grid_pool(
            batch_size, trajectory_rois, src, src_gemoetry, batch_dict, batch_cnt=None
        )
        return proxy_point_geometry, proxy_points

    def get_proposal_aware_motion_feature(
        self, proxy_point, batch_size, trajectory_rois, num_rois, batch_dict
    ):
        ...
        for i in range(num_frames):
            proxy_point_time_padding[
                :, i * self.num_proxy_points : (i + 1) * self.num_proxy_points, -1
            ] = (i * 0.1)

        corner_points, _ = self.get_corner_points_of_roi(trajectory_rois[:, 0, :, :].contiguous())
        corner_points = corner_points.view(batch_size, num_rois, -1, corner_points.shape[-1])
        corner_points = corner_points.view(batch_size * num_rois, -1)
        trajectory_roi_center = trajectory_rois[:, 0, :, :].reshape(batch_size * num_rois, -1)[
            :, :3
        ]
        corner_add_center_points = torch.cat([corner_points, trajectory_roi_center], dim=-1)

        proposal_aware_feat = proxy_point[:, :, :3].repeat(
            1, 1, 9
        ) - corner_add_center_points.unsqueeze(1)

        lwh = (
            trajectory_rois[:, 0, :, :]
            .reshape(batch_size * num_rois, -1)[:, 3:6]
            .unsqueeze(1)
            .repeat(1, proxy_point.shape[1], 1)
        )
        diag_dist = (lwh[:, :, 0] ** 2 + lwh[:, :, 1] ** 2 + lwh[:, :, 2] ** 2) ** 0.5
        proposal_aware_feat = self.spherical_coordinate(
            proposal_aware_feat, diag_dist=diag_dist.unsqueeze(-1)
    ...
    def trajectories_auxiliary_branch(self, trajectory_rois):
        ...
        time_stamp = torch.ones(
            [trajectory_rois.shape[0], trajectory_rois.shape[1], trajectory_rois.shape[2], 1]
        ).cuda()
        ...
        box_reg, box_feat, _ = self.seqboxembed(
            box_seq.permute(0, 2, 3, 1)
            .contiguous()
            .view(batch_rcnn, box_seq.shape[-1], box_seq.shape[1])
        )
    ...
    def generate_trajectory(self, cur_batch_boxes, proposals_list, batch_dict):
        ...
        trajectory_rois = cur_batch_boxes[:, None, :, :].repeat(
            1, batch_dict["rois"].shape[-2], 1, 1
        )
        trajectory_rois[:, 0, :, :] = cur_batch_boxes
        valid_length = torch.zeros(
            [batch_dict["batch_size"], batch_dict["rois"].shape[-2], trajectory_rois.shape[2]]
        )
        valid_length[:, 0] = 1
    ...
    def forward(self, batch_dict):
        ...
        trajectory_rois, valid_length = self.generate_trajectory(
            cur_batch_boxes, proposals_list, batch_dict
        )
        ...
        if self.training:
            ...
            targets_dict["trajectory_rois"][:, batch_dict["cur_frame_idx"], :, :] = batch_dict[
                "rois"
            ]
        ...
        src = self.crop_current_frame_points(
            src, batch_size, trajectory_rois, num_rois, batch_dict
        )

        src = self.crop_previous_frame_points(
            src, batch_size, trajectory_rois, num_rois, valid_length, batch_dict
        )

        src = src.view(batch_size * num_rois, -1, src.shape[-1])

        src_geometry_feature, proxy_points = self.get_proposal_aware_geometry_feature(
            src, batch_size, trajectory_rois, num_rois, batch_dict
        )

        src_motion_feature = self.get_proposal_aware_motion_feature(
            proxy_points, batch_size, trajectory_rois, num_rois, batch_dict
        )
            ...
            if self.avg_stage1_score:
                ...
                if self.model_cfg.get("IOU_WEIGHT", None):
                    ...
                    for bs_idx in range(batch_size):
                        ...
                        batch_cls_preds_car = batch_cls_preds[bs_idx].pow(
                            self.model_cfg.IOU_WEIGHT[0]
                        ) * stage1_score[bs_idx].pow(1 - self.model_cfg.IOU_WEIGHT[0])
                        batch_cls_preds_car = batch_cls_preds_car[car_mask][None]
                        batch_cls_preds_pedcyc = batch_cls_preds[bs_idx].pow(
                            self.model_cfg.IOU_WEIGHT[1]
                        ...
                        box_preds = torch.cat(
                            [
                                batch_dict["batch_box_preds"][bs_idx][car_mask],
                                batch_dict["batch_box_preds"][bs_idx][~car_mask],
                            ],
                            0,
                        )[None]
                        roi_labels = torch.cat(
                            [
                                batch_dict["roi_labels"][bs_idx][car_mask],
                                batch_dict["roi_labels"][bs_idx][~car_mask],
                            ],
                            0,
                        )[None]
    ...
    def get_box_reg_layer_loss(self, forward_ret_dict):
        ...
        if loss_cfgs.REG_LOSS == "smooth-l1":
            ...
            rcnn_loss_reg = (
                rcnn_loss_reg.view(rcnn_batch_size, -1) * fg_mask.unsqueeze(dim=-1).float()
            ).sum() / max(fg_sum, 1)
            rcnn_loss_reg = (
                rcnn_loss_reg
            ...
            if self.model_cfg.USE_AUX_LOSS:
                ...
                if groups != 1:
                    ...
                    for i in range(groups):
                        point_loss_reg = self.reg_loss_func(
                            point_reg[i * slice : (i + 1) * slice]
                            .view(slice, -1)
                            .unsqueeze(dim=0),
                            reg_targets.unsqueeze(dim=0),
                        )
                        point_loss_reg = (
                            point_loss_reg.view(slice, -1) * fg_mask.unsqueeze(dim=-1).float()
                        ).sum() / max(fg_sum, 1)
                        point_loss_reg = (
                            point_loss_reg
                            * loss_cfgs.LOSS_WEIGHTS["rcnn_reg_weight"]
                    ...
                    point_loss_reg = self.reg_loss_func(
                        point_reg.view(rcnn_batch_size, -1).unsqueeze(dim=0),
                        reg_targets.unsqueeze(dim=0),
                    )
                    point_loss_reg = (
                        point_loss_reg.view(rcnn_batch_size, -1)
                ...
                seqbox_loss_reg = self.reg_loss_func(
                    seqbox_reg.view(rcnn_batch_size, -1).unsqueeze(dim=0),
                    reg_targets.unsqueeze(dim=0),
                )
                seqbox_loss_reg = (
                    seqbox_loss_reg.view(rcnn_batch_size, -1) * fg_mask.unsqueeze(dim=-1).float()
                ).sum() / max(fg_sum, 1)
                seqbox_loss_reg = (
                    seqbox_loss_reg
                    * loss_cfgs.LOSS_WEIGHTS["rcnn_reg_weight"]
            ...
            if groups != 1:
                ...
                for i in range(groups):
                    batch_loss_cls = F.binary_cross_entropy(
                        torch.sigmoid(rcnn_cls_flat[i * slice : (i + 1) * slice]),
                    ...
                    rcnn_loss_cls = rcnn_loss_cls + (
                        batch_loss_cls * cls_valid_mask
                    ).sum() / torch.clamp(cls_valid_mask.sum(), min=1.0)
