...
class WaymoDataset(DatasetTemplate):
    ...
    def include_waymo_data(self, mode):
        ...
        use_sequence_data = (
            self.dataset_cfg.get("SEQUENCE_CONFIG", None) is not None
            and self.dataset_cfg.SEQUENCE_CONFIG.ENABLED
        )
    ...
    def load_pred_boxes_to_dict(self, pred_boxes_path):
        self.logger.info(
            f"Loading and reorganizing pred_boxes to dict from path: {pred_boxes_path}"
        )
        ...
        for index, box_dict in enumerate(pred_dicts):
            seq_name = (
                box_dict["frame_id"][:-4].replace("training_", "").replace("validation_", "")
            )
            ...
            pred_labels = np.array(
                [
                    self.class_names.index(box_dict["name"][k]) + 1
                    for k in range(box_dict["name"].shape[0])
                ]
            )
            pred_boxes = np.concatenate(
                (
                    box_dict["boxes_lidar"],
                    box_dict["score"][:, np.newaxis],
        ...
        self.logger.info(
            f"Predicted boxes has been loaded, total sequences: {len(pred_boxes_dict)}"
        )
    ...
    def transform_prebox_to_current(pred_boxes3d, pose_pre, pose_cur):
        ...
        pred_boxes3d[:, 6] = pred_boxes3d[..., 6] + np.arctan2(
    ...
    def reorder_rois_for_refining(pred_bboxes):
        ...
        ordered_bboxes = np.zeros(
            [len(pred_bboxes), num_max_rois, pred_bboxes[0].shape[-1]], dtype=np.float32
        )
    ...
    def get_sequence_data(
        self, info, points, sequence_name, sample_idx, sequence_cfg, load_pred_boxes=False
    ):
        """
        Args:
            info:
        ...
        def remove_ego_points(points, center_radius=1.0):
            mask = ~(
                (np.abs(points[:, 0]) < center_radius) & (np.abs(points[:, 1]) < center_radius)
            )
            return points[mask]
        ...
        sample_idx_pre_list = np.clip(
            sample_idx + np.arange(sequence_cfg.SAMPLE_OFFSET[0], sequence_cfg.SAMPLE_OFFSET[1]),
            0,
            0x7FFFFFFF,
        )
        ...
        if sequence_cfg.get("ONEHOT_TIMESTAMP", False):
            onehot_cur = np.zeros((points.shape[0], len(sample_idx_pre_list) + 1)).astype(
                points.dtype
            )
            onehot_cur[:, 0] = 1
        ...
        for idx, sample_idx_pre in enumerate(sample_idx_pre_list):
            ...
            expand_points_pre = np.concatenate(
                [points_pre[:, :3], np.ones((points_pre.shape[0], 1))], axis=-1
            )
            points_pre_global = np.dot(expand_points_pre, pose_pre.T)[:, :3]
            expand_points_pre_global = np.concatenate(
                [points_pre_global, np.ones((points_pre_global.shape[0], 1))], axis=-1
            )
            ...
            if sequence_cfg.get("ONEHOT_TIMESTAMP", False):
                ...
                points_pre = np.hstack(
                    [
                        points_pre,
                        0.1
                        * (sample_idx - sample_idx_pre)
                        * np.ones((points_pre.shape[0], 1)).astype(points_pre.dtype),
                    ]
                )  # one frame 0.1s
    ...
    def __getitem__(self, index):
        ...
        if (
            ...
            (
                ...
                sequence_name,
                sample_idx,
                self.dataset_cfg.SEQUENCE_CONFIG,
    ...
    def create_groundtruth_database(
        ...
        if use_sequence_data:
            st_frame, ed_frame = (
                self.dataset_cfg.SEQUENCE_CONFIG.SAMPLE_OFFSET[0],
                self.dataset_cfg.SEQUENCE_CONFIG.SAMPLE_OFFSET[1],
            )
            self.dataset_cfg.SEQUENCE_CONFIG.SAMPLE_OFFSET[0] = min(
                -4, st_frame
            ...
            database_save_path = save_path / (
                "%s_gt_database_%s_sampled_%d_multiframe_%s_to_%s"
                % (processed_data_tag, split, sampled_interval, st_frame, ed_frame)
            )
            db_info_save_path = save_path / (
                "%s_waymo_dbinfos_%s_sampled_%d_multiframe_%s_to_%s.pkl"
                % (processed_data_tag, split, sampled_interval, st_frame, ed_frame)
            )
            db_data_save_path = save_path / (
                "%s_gt_database_%s_sampled_%d_multiframe_%s_to_%s_global.npy"
                % (processed_data_tag, split, sampled_interval, st_frame, ed_frame)
            )
        else:
            database_save_path = save_path / (
    ...
    def create_gt_database_of_single_scene(
        self,
        info_with_idx,
    ...
    def create_groundtruth_database_parallel(
        self,
        info_path,
        save_path,
        used_classes=None,
        ...
        if use_sequence_data:
            ...
            st_frame = self.dataset_cfg.SEQUENCE_CONFIG.SAMPLE_OFFSET[0]
            database_save_path = save_path / (
                "%s_gt_database_%s_sampled_%d_multiframe_%s_to_%s_%sparallel"
                % (
                    processed_data_tag,
                    split,
                    sampled_interval,
                    st_frame,
                    ed_frame,
                    "tail_" if crop_gt_with_tail else "",
                )
            )
            db_info_save_path = save_path / (
                "%s_waymo_dbinfos_%s_sampled_%d_multiframe_%s_to_%s_%sparallel.pkl"
                % (
                    processed_data_tag,
                    split,
                    sampled_interval,
        ...
        create_gt_database_of_single_scene = partial(
            ...
            use_sequence_data=use_sequence_data,
        ...
        with multiprocessing.Pool(num_workers) as p:
            all_db_infos_list = list(
                p.map(create_gt_database_of_single_scene, zip(infos, np.arange(len(infos))))
            )
