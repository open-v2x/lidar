...
iou_threshold_dict = {"Car": 0.7, "Bus": 0.7, "Truck": 0.7, "Pedestrian": 0.3, "Cyclist": 0.5}

superclass_iou_threshold_dict = {"Vehicle": 0.7, "Pedestrian": 0.3, "Cyclist": 0.5}
...
def get_evaluation_results(
    ...
    assert difficulty_mode in [
        "Overall&Distance",
        "Overall",
        "Distance",
    ], "difficulty mode is not supported"
    if use_superclass:
        ...
        for diff_idx in range(num_difficulties):
            ...
            for sample_idx in range(num_samples):
                ...
                accum_scores = accumulate_scores(
                    iou, pred_score, gt_flag, pred_flag, iou_threshold=iou_threshold
                )
...
def compute_statistics(iou, pred_scores, gt_flag, pred_flag, score_threshold, iou_threshold):
    ...
    for i in range(num_gt):
        ...
        for j in range(num_pred):
            ...
            iou_ij = iou[i, j]
            if (
                (iou_ij > iou_threshold)
                and (iou_ij > best_matched_iou or gt_assigned_to_ignore)
                and pred_flag[j] == 0
            ):
    ...
    if use_superclass:
        if class_name == "Vehicle":
            reject = np.logical_or(
                pred_anno["name"] == "Pedestrian", pred_anno["name"] == "Cyclist"
            )
        else:
