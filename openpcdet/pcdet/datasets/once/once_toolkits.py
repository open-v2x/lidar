...
class Octopus(object):
    ...
    def val_split_list(self):
        if not osp.isfile(osp.join(self.dataset_root, "ImageSets", "val_set.txt")):
            ...
            val_split_list = set(
                map(lambda x: x.strip(), open(osp.join(self.data_root, "val_set.txt")).readlines())
            )
    ...
    def test_split_list(self):
        if not osp.isfile(osp.join(self.dataset_root, "ImageSets", "test_set.txt")):
            ...
            test_split_list = set(
                map(
                    lambda x: x.strip(), open(osp.join(self.data_root, "test_set.txt")).readlines()
                )
            )
    ...
    def raw_split_list(self):
        if not osp.isfile(osp.join(self.dataset_root, "ImageSets", "raw_set.txt")):
            ...
            raw_split_list = set(
                map(lambda x: x.strip(), open(osp.join(self.data_root, "raw_set.txt")).readlines())
            )
