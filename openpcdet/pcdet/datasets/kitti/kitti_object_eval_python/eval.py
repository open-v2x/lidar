...
def calculate_iou_partly(gt_annos, dt_annos, metric, num_parts=50):
    ...
    for j, num_part in enumerate(split_parts):
        ...
        for i in range(num_part):
            ...
            overlaps.append(
                parted_overlaps[j][
                    gt_num_idx : gt_num_idx + gt_box_num, dt_num_idx : dt_num_idx + dt_box_num
                ]
            )
...
def do_eval(
    gt_annos, dt_annos, current_classes, min_overlaps, compute_aos=False, PR_detail_dict=None
):
...
def get_coco_eval_result(gt_annos, dt_annos, current_classes):
    ...
    for j, curcls in enumerate(current_classes):
        ...
        result += print_str(
            (f"bbox AP:{mAPbbox[j, 0]:.2f}, " f"{mAPbbox[j, 1]:.2f}, " f"{mAPbbox[j, 2]:.2f}")
        )
        result += print_str(
        ...
        if compute_aos:
            result += print_str(
                (f"aos  AP:{mAPaos[j, 0]:.2f}, " f"{mAPaos[j, 1]:.2f}, " f"{mAPaos[j, 2]:.2f}")
            )
    return result
