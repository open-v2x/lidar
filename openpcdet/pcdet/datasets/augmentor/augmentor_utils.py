...
def get_points_in_box(points, gt_box):
    ...
    mask = np.logical_and(
        abs(shift_z) <= dz / 2.0,
        np.logical_and(abs(local_x) <= dx / 2.0 + MARGIN, abs(local_y) <= dy / 2.0 + MARGIN),
    )
...
def get_pyramids(boxes):
    pyramid_orders = np.array(
        [[0, 1, 5, 4], [4, 5, 6, 7], [7, 6, 2, 3], [3, 2, 1, 0], [1, 2, 6, 5], [0, 4, 7, 3]]
    )
    ...
    if swap_pyramid_mask.sum() > 0:
        ...
        if selected_pyramids.sum() > 0:
            ...
            selected_pyramid_indices = [
                np.random.choice(index_j[index_i == i]) if e and (index_i == i).any() else 0
