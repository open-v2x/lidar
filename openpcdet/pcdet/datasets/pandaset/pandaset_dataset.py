...
class PandasetDataset(DatasetTemplate):
    ...
    def _get_annotations(self, info, pose):
        ...
        labels = np.array([self.dataset_cfg.TRAINING_CATEGORIES.get(lab, lab) for lab in labels])
        ...
        for index, box_dict in enumerate(pred_dicts):
            ...
            if output_path is not None:
                ...
                cur_det_file = os.path.join(
                    output_path, seq_id, "predictions", "cuboids", ("{}.pkl.gz".format(frame_id))
                )
            ...
            info = [
                {
                    ...
                    "lidar_path": os.path.join(
                        self.root_path, "dataset", seq, "lidar", ("{:02d}.pkl.gz".format(ii))
                    ),
    ...
    def create_groundtruth_database(self, info_path=None, used_classes=None, split="train"):
        database_save_path = os.path.join(
            self.root_path, "gt_database" if split == "train" else "gt_database_{}".format(split)
        )
        db_info_save_path = os.path.join(self.root_path, "pandaset_dbinfos_{}.pkl".format(split))
