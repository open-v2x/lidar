...
class VectorPoolLocalInterpolateModule(nn.Module):
    ...
    def forward(
        ...
        with torch.no_grad():
            ...
            ) = pointnet2_utils.three_nn_for_vector_pool_by_two_step(
                support_xyz,
                xyz_batch_cnt,
                new_xyz,
                new_xyz_grid_centers,
                new_xyz_batch_cnt,
                self.max_neighbour_distance,
                self.nsample,
                self.neighbor_type,
                self.num_avg_length_of_neighbor_idxs,
                self.num_total_grids,
                self.neighbor_distance_multiplier,
            )
        self.num_avg_length_of_neighbor_idxs = max(
            self.num_avg_length_of_neighbor_idxs, num_avg_length_of_neighbor_idxs.item()
        )
        ...
        interpolated_feats = pointnet2_utils.three_interpolate(
            support_features, idx.view(-1, 3), weight.view(-1, 3)
        )
        ...
        if self.use_xyz:
            near_known_xyz = support_xyz[idx.view(-1, 3).long()].view(
                -1, 3, 3
            )  # ( (M1 + M2 ...)*num_total_grids, 3)
            local_xyz = (new_xyz_grid_centers.view(-1, 1, 3) - near_known_xyz).view(
                -1, idx.shape[1], 9
            )
            if self.xyz_encoding_type == "concat":
                interpolated_feats = torch.cat(
                    (interpolated_feats, local_xyz), dim=-1
                )  # ( M1 + M2 ..., num_total_grids, 9+C)
            else:
                raise NotImplementedError
...
class VectorPoolAggregationModule(nn.Module):
    def __init__(
        ...
        self.num_reduced_channels = (
            input_channels if num_reduced_channels is None else num_reduced_channels
        )
    ...
    def extra_repr(self) -> str:
        ret = (
            f"radius={self.max_neighbour_distance}, local_voxels=({self.num_local_voxel}, "
    ...
    def vector_pool_with_voxel_query(
        ...
        (
            ...
            point_cnt_of_grid,
        ) = pointnet2_utils.vector_pool_with_voxel_query_op(
            xyz,
            xyz_batch_cnt,
            features,
            new_xyz,
            new_xyz_batch_cnt,
            self.num_local_voxel[0],
            self.num_local_voxel[1],
            self.num_local_voxel[2],
            self.max_neighbour_distance,
            self.num_reduced_channels,
            use_xyz,
            self.num_mean_points_per_grid,
            self.neighbor_nsample,
            self.neighbor_type,
            pooling_type,
        )
        self.num_mean_points_per_grid = max(
            self.num_mean_points_per_grid, num_mean_points_per_grid.item()
        )
        ...
        new_features = new_features.view(
            num_new_pts, -1, self.num_reduced_channels
        )  # (N, num_voxel, C)
    ...
    def forward(self, xyz, xyz_batch_cnt, new_xyz, new_xyz_batch_cnt, features, **kwargs):
        ...
        vector_features = vector_features.permute(1, 0)[
            None, :, :
        ]  # (1, num_voxels * C, M1 + M2 ...)
